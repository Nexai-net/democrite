...
namespace Democrite.Framework.Core.UnitTests.Services
{
    ...
    /// <summary>
    /// Unit test <see cref="VGrainIdFactory"/>
    /// </summary>
    public sealed class VGrainIdFactoryUnitTests
    {
        #region Fields
        ...
        /// <summary>
        /// Initializes the <see cref="VGrainIdFactoryUnitTests"/> class.
        /// </summary>
        static VGrainIdFactoryUnitTests()
        {
            var genericBuildIdMethod = typeof(VGrainIdFactoryUnitTests).GetMethod(nameof(BuildId), BindingFlags.NonPublic | BindingFlags.Static);
        ...
        /// <summary>
        /// Ensure that <see cref="VGrainIdFactory"/> could be disposed and public call correctly protected
        /// </summary>
        [Fact]
        public void VGrainIdFactory_DisposableCheck()
        {
            var factory = new VGrainIdFactory();
            using (factory)
            {
        ...
        /// <summary>
        /// Ensure that <see cref="VGrainIdFactory"/> correctly used fallback value of the first id template
        /// </summary>
        [Theory]
        ...
        public void VGrainIdFactory_IdGenerationFromTemplateFallback(Type vgrainType, string expectedId)
        {
            BuildIdFlat(vgrainType, a => string.Equals(a.GetVGrainIdPrimaryValue<string>(), expectedId, StringComparison.OrdinalIgnoreCase));
        }

        /// <summary>
        /// Ensure that <see cref="VGrainIdFactory"/> will correctly recover through fallback value even if correct builder have been put in cache
        /// </summary>
        [Fact]
        public void VGrainIdFactory_ExecutionContextIdProperty_WithCache()
        {
            using (var factory = new VGrainIdFactory())
            {
        ...
        /// <summary>
        /// Call <see cref="IVGrainIdFactory.BuildNewId{TVGrain}(object?, IExecutionContext)"/> and compare result
        /// </summary>
