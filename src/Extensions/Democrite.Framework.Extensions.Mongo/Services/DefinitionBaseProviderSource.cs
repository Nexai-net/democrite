...
namespace Democrite.Framework.Extensions.Mongo.Services
{
    ...
    /// </summary>
    public abstract class DefinitionBaseProviderSource<TDocumentType> : ProviderStrategyBaseSource<TDocumentType, Guid>, IProviderStrategySource<TDocumentType, Guid>, IDefinitionSourceProvider<TDocumentType>
        where TDocumentType : class, IDefinition
    {
            ...
            : base(serviceProvider)
        {
            ...
            var repository = new MongoDefinitionReadOnlyRepository<TDocumentType>(mongoClientFactory, serviceProvider, mongoDBOptions: mongoDBOptions);

            this._repository = repository;
            this._repositoryContainer = repository;
        ...
        protected override ValueTask OnProviderInitializedAsync(IServiceProvider? _, CancellationToken token)
        {
            if (this._repository is ISupportInitialization<string> initRepo)
                return initRepo.InitializationAsync(this._key, token);
