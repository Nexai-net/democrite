...
namespace Democrite.Framework.Node.Storages
{
    ...
    [DebuggerDisplay("Memory Repository {s_entityTraits} : {_storageName}")]
    internal class MemoryReadOnlyRepository<TEntity, TRegistryGrain> : IReadOnlyRepository<TEntity>
        where TRegistryGrain : IMemoryStorageRegistryGrain
    {
        #region Fields
                                        ...
                                        IDemocriteSerializer democriteSerializer)
        {
            ...
            this.RegistryGrain = grainFactory.GetGrain<TRegistryGrain>(storageName);
        }
        ...
        /// </summary>
        protected TRegistryGrain RegistryGrain { get; }
        ...
        {
            var compiledFilter = filter?.Compile() ?? s_allTrue;
            var entities = await GetAllImplAsync(token);

            return entities?.Where(compiledFilter).ToArray() ?? EnumerableHelper<TEntity>.ReadOnlyArray;
        }
        ...
        /// </summary>
        protected async Task<IEnumerable<TEntity>?> GetAllImplAsync(CancellationToken token)
        {
        ...
        /// Download all compatible values and return a enumeration that could be filtered
        /// </summary>
        protected async Task<IEnumerable<TEntity>?> GetAllImplAsync<TKey>(IReadOnlyCollection<TKey> keys, CancellationToken token)
            where TKey : notnull, IEquatable<TKey>
        {
            using (var cancelSource = new GrainCancellationTokenSource())
            {
                ...
                // TODO : Add state filter
                var entitiesByBytes = await this.RegistryGrain.AsReference<IMemoryStorageRegistryGrain<TKey>>().GetAllStoreByKeysDataAsync(this.StateName, keys, cancelSource.Token);
    ...
    [DebuggerDisplay("Memory Repository {s_entityTraits}[{s_entityIdTraits}] : {_storageName}")]
    internal class MemoryReadOnlyRepository<TEntity, TEntityId, TRegistryGrain> : MemoryReadOnlyRepository<TEntity, TRegistryGrain>, IReadOnlyRepository<TEntity, TEntityId>
